...
impl Vmm {
    ...
    fn add_block_device(&mut self, cfg: &BlockConfig) -> Result<()> {
        self.add_block_device_internal(cfg, false, false)
    }

    /// Add an extra block device (e.g. seed.iso) after VMM creation.
    pub fn add_block_device_extra(&mut self, path: PathBuf, read_only: bool) -> Result<()> {
        let cfg = BlockConfig { path };
        self.add_block_device_internal(&cfg, read_only, false)
    }

    fn add_block_device_internal(
        &mut self,
        cfg: &BlockConfig,
        read_only: bool,
        root_device: bool,
